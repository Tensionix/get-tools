...
set "MENU_FILE=%RUNTIME_DIR%\installapps_menu.txt"
set "RES_FILE=%RUNTIME_DIR%\installapps_menu_res.txt"
...
:FZF_MENU
>  "%MENU_FILE%" echo [01] Install MSVC 2015+ (x86/x64)       ^| msvc2015    ^| VCRedist.2015+.x86 + x64
>> "%MENU_FILE%" echo [02] Install MSVC Legacy (2005-2013)    ^| msvc_legacy ^| 2005 2008 2010 2013
>>"%MENU_FILE%" echo.
>> "%MENU_FILE%" echo [03] Install Docker Desktop             ^| docker      ^| Docker.DockerDesktop  [interactive]
>> "%MENU_FILE%" echo [04] Install Node.js LTS                ^| nodejs      ^| OpenJS.NodeJS.LTS
>> "%MENU_FILE%" echo [05] Install Obsidian                   ^| obsidian    ^| Obsidian.Obsidian
>> "%MENU_FILE%" echo [06] Install PowerShell 7+              ^| powershell  ^| Microsoft.PowerShell
>> "%MENU_FILE%" echo [07] Install Python 3.12                ^| python      ^| Python.Python.3.12
>>"%MENU_FILE%" echo.
>> "%MENU_FILE%" echo [08] Install Shutter Encoder 19.8       ^| shutter     ^| PaulPacifico.ShutterEncoder  pinned version
>> "%MENU_FILE%" echo [09] Install Shutter Encoder 19.8 + PIN ^| shutter_pin ^| install then pin to 19.8
>> "%MENU_FILE%" echo [10] Install Windows Terminal           ^| winterminal ^| Microsoft.WindowsTerminal
>> "%MENU_FILE%" echo [11] Install LosslessCut                ^| losslesscut ^| mifi.losslesscut
>>"%MENU_FILE%" echo.
>> "%MENU_FILE%" echo [00] 🚪 Exit                            ^| exit        ^| close

"%FZF_CMD%" --bind="backspace:abort" --prompt="audion@install-apps > " --pointer=">" --header="Choose an app to install:" --layout=reverse --border=rounded --info=hidden --margin=1,2 < "%MENU_FILE%" > "%RES_FILE%"
...
:FALLBACK_MENU
echo [1]  Install MSVC 2015+ (x86/x64)
echo [2]  Install MSVC Legacy (2005-2013)
echo [3]  Install Docker Desktop
echo [4]  Install Node.js LTS
echo [5]  Install Obsidian
echo [6]  Install PowerShell 7+
echo [7]  Install Python 3.12
echo [8]  Install Shutter Encoder 19.8
echo [9]  Install Shutter Encoder 19.8 + Pin
echo [A]  Install Windows Terminal
echo [B]  Install LosslessCut
echo [0]  Exit
echo.
...
:INSTALL_MSVC2015
call :CONFIRM_RUN "Install MSVC 2015+ x86/x64"
...
:INSTALL_MSVC_LEGACY
call :CONFIRM_RUN "Install MSVC Legacy 2005-2013"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_DOCKER
call :CONFIRM_RUN "Install Docker Desktop"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_NODEJS
call :CONFIRM_RUN "Install Node.js LTS"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_OBSIDIAN
call :CONFIRM_RUN "Install Obsidian"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_POWERSHELL
call :CONFIRM_RUN "Install PowerShell 7+"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_PYTHON
call :CONFIRM_RUN "Install Python 3.12"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_SHUTTER
call :CONFIRM_RUN "Install Shutter Encoder 19.8"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_SHUTTER_PIN
call :CONFIRM_RUN "Install Shutter Encoder 19.8 and pin it"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
call "%ROOT%system_core\winget\install_apps\Install-ShutterEncoder-19.8.cmd"
...
:INSTALL_WINTERMINAL
call :CONFIRM_RUN "Install Windows Terminal"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
:INSTALL_LOSSLESSCUT
call :CONFIRM_RUN "Install LosslessCut"
if errorlevel 2 goto END
if errorlevel 1 goto MAIN
...
echo [INFO] Press any key to return to the install menu.
