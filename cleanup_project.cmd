...
title Audion Get Tools - Cleanup
...
if not exist "%BASE_DIR%\system_core\winget\scripts" (
  echo [ERROR] Project marker not found:
  echo %BASE_DIR%\system_core\winget\scripts
  echo.
  echo Cleanup aborted.
  call :WAIT_KEY
  exit /b 1
...
echo ======================================================================
echo   AUDION GET TOOLS - LEAN SOURCE CLEANUP
echo ======================================================================
echo Root:
echo   %BASE_DIR%
echo.
echo This script returns the project to a small source-only / LEAN state.
echo It keeps source scripts, launchers, docs, permanent configs and policies.
echo It removes generated workspace content, build/runtime payloads and caches:
echo   - input, output, logs, report, workspace and release contents
...
echo.
echo Protected: config, WinGet scripts, GUI/core source, launchers, install scripts,
echo            README/policies and package list files.
echo.
