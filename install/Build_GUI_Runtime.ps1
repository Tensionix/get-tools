...
$requirementsFile = Join-Path $installDir "requirements_gui.in"
$doctorScript = Join-Path $rootDir "system_core\doctor.py"
$guiAppScript = Join-Path $rootDir "system_core\ui_nicegui\app.py"
...
Write-Host "======================================================================"
Write-Host "AUDION GET TOOLS - BUILD GUI RUNTIME (PS)"
Write-Host "======================================================================"
...
Invoke-Checked -Label "Verifying environment" -Command { & $pythonExe $doctorScript }
if (Test-Path $guiAppScript) {
    Invoke-Checked -Label "Running GUI smoke check" -Command { & $pythonExe $guiAppScript --smoke }
}
