@echo off
setlocal EnableExtensions
chcp 65001 >nul
set "EXIT_RC=0"

rem Messages and comments are intentionally kept in English.

where winget >nul 2>nul
if errorlevel 1 (
    echo [ERROR] winget was not found in PATH.
    echo [INFO] Update App Installer from Microsoft Store and try again.
    set "EXIT_RC=1"
    goto :end
)

echo ================================================================
echo AUDION POWERSHELL UPDATE
echo ================================================================
echo [INFO] This script updates PowerShell 7+ only.
echo [INFO] If PowerShell 7+ is not installed yet, it will be installed first.
echo.

where pwsh >nul 2>nul
if errorlevel 1 (
    echo [INFO] PowerShell 7+ is not installed. Installing it first...
    winget install --id Microsoft.PowerShell -e --source winget --accept-source-agreements --accept-package-agreements --no-upgrade
) else (
    winget upgrade --id Microsoft.PowerShell -e --source winget --accept-source-agreements --accept-package-agreements
)
set "RC=%ERRORLEVEL%"

if "%RC%"=="0" (
    echo [OK] PowerShell processed successfully.
    goto :end
)

if "%RC%"=="2" (
    echo [CANCELLED] PowerShell update was cancelled.
    goto :end
)

if "%RC%"=="-1978335136" (
    echo [INFO] No update available for PowerShell.
    goto :end
)

if "%RC%"=="-1978335189" (
    echo [INFO] No update available for PowerShell.
    goto :end
)

if "%RC%"=="-1978335212" (
    echo [INFO] No update available or no installed match for PowerShell.
    goto :end
)

echo [WARN] PowerShell update returned errorlevel %RC%.
set "EXIT_RC=%RC%"

:end
echo.
echo [INFO] Press any key to close this window.
if not defined AUDION_NO_PAUSE pause
endlocal & exit /b %EXIT_RC%
